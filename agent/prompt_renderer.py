...
def render_taskboard_fire_prompt(role: str, task: dict[str, Any]) -> str:
    ...
    Args:
        ...
        task: Taskboard task payload from the taskboard API or dispatcher.
...
def _extract_substitutions(
    ...
    *,
    ...
    """Flatten taskboard fields into template substitutions.
    ...
    task_id = _stringify(_field(task_mapping, "task_id", "taskId", "id")).strip()
    ...
    branch_name = _stringify(
        _field(task_mapping, "branch_name", "branchName", "branch")
    ).strip()
    ...
    if not output_target:
        role_slug = _slugify(agent_id or role_name, fallback="agent")
    ...
    return {
        ...
        "agent": _stringify(_field(task_mapping, "agent", "agent_name", "agentName")),
        "agent_id": agent_id,
        ...
        "repo_url": _stringify(
            _field(task_mapping, "repo_url", "repoUrl")
            or _field(project, "repo_url", "repoUrl")
        ),
        ...
        "project_context": _stringify(
            _field(task_mapping, "project_context", "projectContext") or project
        ),
        "epic_context": _stringify(
            _field(task_mapping, "epic_context", "epicContext") or epic
        ),
    }
...
def _resolve_template_path(prompt_root: Path, role: str) -> Path:
...
def _cap_prompt(rendered: str, *, stop_marker: str) -> str:
