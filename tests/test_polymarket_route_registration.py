...
def test_polymarket_route_loads_from_agent_config() -> None:
    ...
    assert router.channels["polymarket_alarms"].subjects == ["alerts.>"]


def test_polymarket_spawn_agent_action_wiring_is_canonical() -> None:
    config = json.loads(Path(CONFIG_PATH).read_text(encoding="utf-8"))
    router = SignalRouter(config["daemon"]["signal_router"])
    action = router.routes["polymarket-alarm-response"].actions[0]

    assert action.kind == "spawn_agent"
    assert action.params["pack"] == "kai-alert-response"
