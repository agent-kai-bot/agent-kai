#!/usr/bin/env python3
"""
RSI Divergence Reversal Monitor - Simplified Production Version
...
class RSIDivergenceMonitor:
    ...
    def get_rsi(self) -> float:
        ...
        if isinstance(rsi_data, str):
            # Extract RSI value from string output
            import re
    ...
    def get_macd(self) -> Tuple[float, float]:
        ...
        # MACD might return empty or structured data
        if isinstance(macd_data, dict) and 'values' in macd_data:
            values = macd_data['values']
            if len(values) >= 1:
                macd_line = values[-1] if isinstance(values[-1], (int, float)) else 0.0
                signal_line = values[-1] if isinstance(values[-1], (int, float)) else 0.0
    ...
    def get_close_price(self) -> float:
        ...
        if isinstance(ohlcv_data, dict) and 'candles' in ohlcv_data:
        ...
        price_str = get_latest_price(symbol=self.symbol)
        import re
