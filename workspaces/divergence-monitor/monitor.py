...
Publishes alerts to NATS subject: agent.alerts.divergence
"""
...
from agent.crypto_tools import (
    calculate_indicator,
    get_latest_price,
    query_ohlcv
)
...
class RSIDivergenceMonitor:
    def __init__(self):
        ...
        # NATS connection
        self.nats: Optional[NatsBus] = None
    ...
    def connect_nats(self):
        """Connect to NATS"""
        try:
            self.nats = NatsBus(url="nats://localhost:4222", agent_name="divergence-monitor")
            self.nats.connect_sync()
    ...
    def get_rsi(self) -> float:
        ...
        rsi_data = calculate_indicator.invoke({
            'symbol': self.symbol,
            'indicator': 'RSI',
        ...
        if isinstance(rsi_data, str):
            match = re.search(r'RSI\(14\) = ([\d.]+)', rsi_data)
            if match:
    ...
    def get_ema(self) -> float:
        ...
        ema_data = calculate_indicator.invoke({
            'symbol': self.symbol,
            'indicator': 'EMA',
        ...
        if isinstance(ema_data, str):
            match = re.search(r'EMA\(\d+\) = \$([\d,]+\.?\d*)', ema_data)
            if match:
    ...
    def get_macd(self) -> Tuple[float, float]:
        ...
        macd_data = calculate_indicator.invoke({
            'symbol': self.symbol,
            'indicator': 'MACD',
            'interval': self.interval,
            'limit': 20
        ...
        if isinstance(macd_data, str):
            macd_match = re.search(r'MACD = ([\d.\-]+)', macd_data)
            signal_match = re.search(r'Signal = ([\d.\-]+)', macd_data)
    ...
    def get_close_price(self) -> float:
        ...
        price_data = get_latest_price.invoke({'symbol': self.symbol})
    ...
    def publish_alert(self, signal_type: str, rsi: float, ema: float, macd: float,
        ...
        if self.nats:
            self.nats.publish('agent.alerts.divergence', json.dumps(alert))
            print(f"\n🚨 [{signal_type}] Alert published for {self.symbol}")
        else:
            print(f"\n🚨 [{signal_type}] Alert (NATS not connected):")
