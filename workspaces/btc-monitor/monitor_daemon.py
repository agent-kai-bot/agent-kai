#!/usr/bin/env python3
"""
BTC Multi-Timeframe Alignment Monitor - API-based Daemon
Monitors BTC across 15m, 1h, 4h timeframes for alignment signals.
...
def query_ohlcv(symbol, interval, limit=50):
    """Query OHLCV via API"""
    try:
        ...
        log(f"Error querying OHLCV: {e}")
    return None

def calculate_indicator(symbol, indicator, period, interval, limit=200):
    """Calculate indicator via API"""
    try:
        url = f"{API_BASE}/indicator"
        params = {"symbol": symbol, "indicator": indicator, "period": period, "interval": interval, "limit": limit}
        response = requests.get(url, params=params, timeout=10)
        if response.status_code == 200:
            return response.json()
    except Exception as e:
        log(f"Error calculating indicator: {e}")
    return None

def nats_publish(subject, message):
    """Publish to NATS via API"""
    try:
        url = f"{API_BASE}/nats/publish"
        data = {"subject": subject, "message": message}
        response = requests.post(url, json=data, timeout=10)
        return response.status_code == 200
    except Exception as e:
        log(f"Error publishing to NATS: {e}")
        return False
...
def publish_alert(signal_type, entry_price, stop_loss, first_target, invalidation_level, atr_15m, timeframes_data):
    ...
    # Publish to NATS
    success = nats_publish(NATS_SUBJECT, json.dumps(alert))
